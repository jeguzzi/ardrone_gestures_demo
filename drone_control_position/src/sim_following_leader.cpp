...
#include <gesture_msgs/leader_info.h>
...
gesture_msgs::leader_info leader_publish_data;
...
double Leader_info[4] = {0.0,0.0,0.0,0.0}; /* x,y,z,yaw */
...
void hasReceivedModelState(const optitrack_msgs::RigidBodies::ConstPtr& msg){
	...
	Leader_info[3] = quaternion2angles(quaternion);

	leader_publish_data.LeaderYaw = Leader_info[3];
	leader_publish_data.LeaderX = Leader_info[0];
...
ros::init(argc, argv, "sim_following_leader_node");
ros::NodeHandle nh_;
ros::Rate rate(20.0);
...
ros::Publisher leader_info_pub_=nh_.advertise<gesture_msgs::leader_info>("/leader_info", 1);
...
std_msgs::Empty EmergencyMsg;
	...
	while (ros::ok()){
	nh_.getParam("/drone_control_node/leader_distance",dLeader);
