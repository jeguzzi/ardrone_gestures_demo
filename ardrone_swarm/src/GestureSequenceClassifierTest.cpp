...
static long int wait_time_before_start = 2*1000000; //5 in millisecs
static long int time_between_gestures = 25*1000000;  //8 in millisecs
static long int gesture_capturing_time = 4*1000000; //4 in millisecs
static long int time_between_indrobots = 15*1000000; //5 in millisecs
...
CvFont font;
CvPoint bodyline;
double motiondata[NFRAMES][NDATA];
int resultduration, resultfingercounts, facescore, individualctr = 0;
int resultint, resultyou, resultaction, resultdistance, faceheight, framectr;
...
//Yellow
static int Hmin_Y = 18;
static int Hmax_Y = 28;
static int Smin_Y = 150;
static int Smax_Y = 255;
static int Vmin_Y = 10;
static int Vmax_Y = 255;
//Green
static int Hmin_G = 45;
static int Hmax_G = 70;
static int Smin_G = 70;
static int Smax_G = 255;
static int Vmin_G = 10;
static int Vmax_G = 190;
//Orange
static int Hmin_O1 = 0;
static int Hmax_O1 = 18;
static int Hmin_O2 = 175;
static int Hmax_O2 = 180;
static int Smin_O = 160;
static int Smax_O = 255;
static int Vmin_O = 130;
static int Vmax_O = 255;
...
void computeallfeatures(CBlobResult blobs, CBlob biggestBlob)
{
...
**/
    ...
    {
		//perform normalization, based on normalization values for each individual classifier
		features[i] = lowerlimit + (upperlimit-lowerlimit) * (features[i] - featuresmin[i]) / (featuresmax[i] - featuresmin[i]);
...
void computeblobfeatures(IplImage *image, CBlob blob)
{
	...
	computeallfeatures(finalblobs, biggestfinalBlob);
...
int segmentandpredict(int glove)
{
	...
	if (glove == 1) {
		...
		computeblobfeatures(imageYellow, biggestYellowBlob);
		//Predict using SVM
		predicted_label = svm_do_prediction_passmemory(modelfile,features);
	...
	// GREEN blob //
		...
		//get final segmented blob and compute features
		computeblobfeatures(imageGreen, biggestGreenBlob);
		//Predict using SVM
		predicted_label = svm_do_prediction_passmemory(modelfile,features);
...
int computepredictionresults()
{
	//Initialize
	int gloveg, glovey, total;

	//Segment, compute features and predict
	glovey = segmentandpredict(1); // yellow glove
	gloveg = segmentandpredict(2); // green glove
...
void *gesture_sequence(void *ptr)
{
	...
	while (1) {
		...
		//To get face detector running need initial face estimates
		directionshown = TRUE;	//no recording just dummy for face detector initialization
		usleep(100);
		directionshown = FALSE;	//no recording just dummy for face detector initialization
		...
		usleep(time_between_gestures);

		//Start capturing images for addressing
		recording = TRUE;
		gesturemode="(1) **RECORDING** ADDRESSING";
		usleep(gesture_capturing_time);
		recording = FALSE;
		...
		//classify if the user is presenting a finger count or direction.
			...
			gesturemode="SOME ROBOTS: [<DIRECTION|#ROBOTS>]";
			usleep(time_between_gestures);

			//start capturing images for addressing
			//classify if it is a direction or a fingercount
			recording = TRUE;
			gesturemode="**RECORDING** SOME ROBOTS";
			usleep(gesture_capturing_time);
			recording = FALSE;


			// if both gloves are upper than jacket lowerbound and 2-blobs ==> DIRECTION
			// ELSE ==> #ROBOTS
			if ((distancegloves > 0) && (centroidgreen.y < jacketlbound.y) && (centroidyellow.y < jacketlbound.y)) { //DIRECTION
				//use a delay before acquiring new gestures
				gesturemode="SOME ROBOTS: [<DIRECTION USING BOTH HANDS>]";
				usleep(time_between_gestures);

				//start capturing images for addressing
				recording = TRUE;
				directionshown = TRUE;
				gesturemode="**RECORDING** SOME ROBOTS: DIRECTION";
				usleep(gesture_capturing_time);
				recording = FALSE;
				directionshown = FALSE;

				// get direction of both green and yellow gloves in 360 degree plain
				char somestr1[35];
			...
			// ELSE ==> #ROBOTS
				...
				//use a delay before acquiring new gestures
				gesturemode="SOME ROBOTS: [<#ROBOTS>]";
				usleep(time_between_gestures);

				//start capturing images for addressing
				recording = TRUE;
				gesturemode="**RECORDING** SOME ROBOTS (#ROBOTS)";
				usleep(gesture_capturing_time);
				recording = FALSE;

				// process frames and get result
				resultfingercounts=computepredictionresults();
		...
		//
			...
			recording = TRUE;	//no actual recording just dummy
			...
			recording = FALSE;	//no actual recording just dummy
			...
			while(1)
			{
				...
				//Wait, since the human wants to point again to another robot
				usleep(time_between_indrobots);
				//record gesture
				recording = TRUE;
				usleep(gesture_capturing_time);
		...
		if ((distancejacket == 0) && (distancegloves == 0)) {  //&& (centroidgreen.y < faceheight) && (centroidyellow.y < faceheight)) {
			...
			//capture gestures for subactions
			recording = TRUE;
			directionshown = TRUE;
			gesturemode="(3) **RECORDING** DIRECTION";
			usleep(gesture_capturing_time);
			recording = FALSE;
			directionshown = FALSE;
			...
			usleep(time_between_gestures);
			recording = TRUE;
			gesturemode="(4) **RECORDING** DURATION";
			usleep(gesture_capturing_time);
			recording = FALSE;
				
			//classify distance in "metres" (based on finger counts)
			resultduration=computepredictionresults();
			// determine classified result
			sprintf(resultfourth, "FOR %d MINUTES", resultduration);
			...
			recording = TRUE;
			directionshown = TRUE;
			gesturemode="(3) **RECORDING** PERSON DIRECTION";
			usleep(gesture_capturing_time);
			directionshown = FALSE;
			// get direction of hand in 360 degree plain
			...
			usleep(time_between_gestures);
			recording = TRUE;
			gesturemode="(4) **RECORDING** DISTANCE";
			usleep(gesture_capturing_time);
			recording = FALSE;	
			
			//classify distance in "metres" (based on finger counts)
			resultdistance=computepredictionresults();
			// determine classified result
			sprintf(resultfourth, "AT %d METRES", resultdistance);
		...
		gesturemode = "PROGRAM FINISHED";
		usleep(time_between_gestures);
		threadstart = FALSE;

		//empty and clear results
		resultfirst = "";
		resultsecond = "";
		*resultthird = '\0';
		*resultfourth = '\0';
...
/**
	// Declare variables
	int currentsaturation, width, height, nchannels, step;
	int saturationthreshold = (framewidth * frameheight) * 0.05; // Less than 5% of pixels are saturated
**/

	// Capture image
	CvCapture* capture = cvCaptureFromCAM(0);
	//CvCapture* capture = cvCreateCameraCapture(1); //old function
/**
	if (!capture) {
...
**/
	...
	double framewidth = cvGetCaptureProperty(capture, CV_CAP_PROP_FRAME_WIDTH);
	double frameheight = cvGetCaptureProperty(capture, CV_CAP_PROP_FRAME_HEIGHT);
	...
	double gain = cvGetCaptureProperty(capture, CV_CAP_PROP_GAIN);
	double exposure = cvGetCaptureProperty(capture, CV_CAP_PROP_EXPOSURE);
	double convertrgb = cvGetCaptureProperty(capture, CV_CAP_PROP_CONVERT_RGB);
	double rectification = cvGetCaptureProperty(capture, CV_CAP_PROP_RECTIFICATION);
	//double whitebalance = cvGetCaptureProperty(capture, CV_CAP_PROP_WHITE_BALANCE);
	...
	printf("Gain: %f\n", gain);
	printf("Exposure: %f\n", exposure);
	printf("Convert RGB: %f\n", convertrgb);
	printf("Rectification: %f\n", rectification);
	printf("--------\n");
/**
...
**/
	...
	// Important variables used in the main while loop
	CBlob currentBlob;
	IplImage *displayImageflip;
	CvPoint finalpt1, finalpt2, facecentroid;
	double firstsizeyg, secondsizeyg, firstsizeo, secondsizeo, d1, d2, d3, d4, d5, d6, d7, d8, d9;
	int straight, right, left, frontfacescore, frontfaceflipscore, sidefacescore, sidefaceflipscore, samplecount;
	CvPoint centroidyellow2, centroidgreen2, centroidjacket2, jacketubound2, jacketlbound2, jacketright2, jacketleft2;
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// Compute NFRAME values for all these metrics and average them by dividing by NFRAMES
	double distgreenjacketubound[NFRAMES], distgreenjacketcentroid[NFRAMES], distgreenjacketlbound[NFRAMES], distyellowjacketubound[NFRAMES];
	double distyellowjacketcentroid[NFRAMES], distyellowjacketlbound[NFRAMES], distjacketuboundlbound[NFRAMES], distjacketleftright[NFRAMES], distgreenyellow[NFRAMES];
	...
	while (1)
	{
		...
		displayImage = cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 3);
		cvCopy(frame,displayImage,NULL);
		// Create a flipped copy of the image (flip, left and right)
		displayImageflip = cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 3);
...
**/	
		...
		//superimpose classified result on displayed frames
		if (threadstart==TRUE) {
...
/**
		// Check if image is oversatured in R, G and B channels
		int ctrR = 1, ctrG = 1, ctrB = 1;
		width		= displayImage->width;
		height		= displayImage->height;
		nchannels	= displayImage->nChannels;
		step		= displayImage->widthStep;
		uchar *data = (uchar*)displayImage->imageData;    	

		int x, y;
		for(x = 0; x < frameheight; x++) {
			for(y = 0; y < framewidth; y++) {
				if ((data[x*step + y*nchannels + 0] == 255)) {
					ctrR++;
				} else if ((data[x*step + y*nchannels + 1] == 255)) {
					ctrG++;
				} else if ((data[x*step + y*nchannels + 2] == 255)) {
					ctrB++;
				}
			}
		}
		...
		// Adjust saturation
		if ((ctrR-1 > saturationthreshold) || (ctrG-1 > saturationthreshold) || (ctrB-1 > saturationthreshold)) {
			// Autocorrect if image is oversaturated
			currentsaturation = cvGetCaptureProperty(capture, CV_CAP_PROP_GAIN);
			cvSetCaptureProperty(capture, CV_CAP_PROP_GAIN, currentsaturation-1);
			printf("Saturation Adjustment: %d\n", currentsaturation-1);
		}
**/

		/**
		- First set exposure time to about 0.01 seconds
		- If you have control of white balance, disable auto white balancing and set it to some default (it's not very important)
		- Then set gain in such a way that few pixels are saturated (say, less than 1% of the pixels should have R,G or B at 255).
		The last point is really the only important one.

		Finally, get a gray object and compute its average RGB values. Let them be R, G and B.  Let M be the maximum of R, G and B.
		To properly set the white balance of an image, multiply its red values by R/M; its green values by G/M; its blue values by B/M.
		**/
		...
		if (directionshown == TRUE) {
			...
			if (frontfacescore > sidefacescore) {
				finalpt1.x = facefront.x;
				finalpt1.y = facefront.y;
				finalpt2.x = facefront.width;
				finalpt2.y = facefront.height;
//				cvRectangle(displayImage, finalpt1, finalpt2, CV_RGB(255,0,0), 3, 4, 0);


			// If only side pose of face is detected
			} else if (sidefacescore > frontfacescore) {
				finalpt1.x = faceside.x;
				finalpt1.y = faceside.y;
				finalpt2.x = faceside.width;
				finalpt2.y = faceside.height;
...
//			cvLine(displayImage, facecentroid, bodyline, CV_RGB(0,255,255), 1, 4, 0);
			...
			//////// Orientation of Face /////////
			if ((straight <= 50) && (right <= 3) && (left <= 3)) {
				facedirection = "NO HUMAN PRESENT";
			} else if ((straight > 0) && (straight < 50) && (left <= 15)) {
				facedirection = "RIGHT ORIENTED";
			} else if ((straight >= 50) && (right >= 2) && (left >= 2)) {
				facedirection = "IN THE CENTER";
			} else if ((straight > 0) && (straight < 50) && (right <= 15)) {
		...
		Point ptYellow1, ptYellow2, ptGreen1, ptGreen2, ptOrange11, ptOrange12, ptOrange21, ptOrange22, ptYellowGreen1, ptYellowGreen2, ptOrangeSplit1, ptOrangeSplit2;
		IplImage *imageHSV					= cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 3);
		IplImage *imageYellow				= cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);   // Grayscale output image
		IplImage *imageGreen				= cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);   // Grayscale output image
		IplImage *imageOrange				= cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);   // Grayscale output image
		IplImage *imageOrange1				= cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);   // Grayscale output image
		IplImage *imageOrange2				= cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);   // Grayscale output image
		...
		cvInRangeS(imageHSV, cvScalar(Hmin_G, Smin_G, Vmin_G), cvScalar(Hmax_G, Smax_G, Vmax_G), imageGreen); // green threshold
		// Segement ORANGE //
		cvInRangeS(imageHSV, cvScalar(Hmin_O1, Smin_O, Vmin_O), cvScalar(Hmax_O1, Smax_O, Vmax_O), imageOrange1); // orange threshold
		cvInRangeS(imageHSV, cvScalar(Hmin_O2, Smin_O, Vmin_O), cvScalar(Hmax_O2, Smax_O, Vmax_O), imageOrange2); // orange threshold
		cvOr(imageOrange1,imageOrange2,imageOrange); // Combine both orange blobs
		...
		ptOrangeSplit1.x = max(LargestOrangeBlob.MinX(),SecondLargestOrangeBlob.MinX());
		ptOrangeSplit1.y = max(LargestOrangeBlob.MinY(),SecondLargestOrangeBlob.MinY());
		ptOrangeSplit2.x = max(LargestOrangeBlob.MaxX(),SecondLargestOrangeBlob.MaxX());
		ptOrangeSplit2.y = max(LargestOrangeBlob.MaxY(),SecondLargestOrangeBlob.MaxY());
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// Copy all this data into memory
		motiondata[framectr][0]  = centroidyellow.x;
		motiondata[framectr][1]  = centroidgreen.x;
		motiondata[framectr][2]  = centroidjacket.x;
		motiondata[framectr][3]  = centroidyellow.y;
		motiondata[framectr][4]  = centroidgreen.y;
		motiondata[framectr][5]  = centroidjacket.y;
		motiondata[framectr][6]  = jacketubound.x;
		motiondata[framectr][7]  = jacketubound.y;
		motiondata[framectr][8]  = jacketlbound.x;
		motiondata[framectr][9]	 = jacketlbound.y;
		motiondata[framectr][10] = jacketleft.x;
		motiondata[framectr][11] = jacketleft.y;
		motiondata[framectr][12] = jacketright.x;
		motiondata[framectr][13] = jacketright.y;
		...
		if (secondsizeo > jacketsizeratio*firstsizeo) {  // 2 Blobs
			...
			cvRectangle( displayImage, ptOrangeSplit1, ptOrangeSplit2, CV_RGB(255,127,0), 1, 4, 0);  //1 Blob
			distancejacket = 0;
		}
		/////////////////////////////////////////////////////////////////////////////////////
		...
		if (individualctr > 0) {
			...
			cvPutText(displayImage,tempind,cvPoint(300,680), &font, CV_RGB(0,255,0));
		}
		...
		for (int j=0; j<NFRAMES; j++) {
			centroidyellow2.x = motiondata[j][0];
			centroidgreen2.x  = motiondata[j][1];
			centroidjacket2.x = motiondata[j][2];
			centroidyellow2.y = motiondata[j][3];
			centroidgreen2.y  = motiondata[j][4];
			centroidjacket2.y = motiondata[j][5];
			jacketubound2.x   = motiondata[j][6];
			jacketubound2.y   = motiondata[j][7];
			jacketlbound2.x   = motiondata[j][8];
			jacketlbound2.y   = motiondata[j][9];
			jacketleft2.x     = motiondata[j][10];
			jacketleft2.y     = motiondata[j][11];
			jacketright2.x    = motiondata[j][12];
			jacketright2.y    = motiondata[j][13];

			// Compute distance measures
			distgreenjacketubound[j] = sqrt(abs((centroidgreen2.x - jacketubound2.x)^2 + (centroidgreen2.y - jacketubound2.y)^2));
			distgreenjacketcentroid[j] = sqrt(abs((centroidgreen2.x - centroidjacket2.x)^2 + (centroidgreen2.y - centroidjacket2.y)^2)); //
			distgreenjacketlbound[j] = sqrt(abs((centroidgreen2.x - jacketlbound2.x)^2 + (centroidgreen2.y - jacketlbound2.y)^2));
			distyellowjacketubound[j] = sqrt(abs((centroidyellow2.x - jacketubound2.x)^2 + (centroidyellow2.y - jacketubound2.y)^2));
			distyellowjacketcentroid[j] = sqrt(abs((centroidyellow2.x - centroidjacket2.x)^2 + (centroidyellow2.y - centroidjacket2.y)^2)); //
			distyellowjacketlbound[j] = sqrt(abs((centroidyellow2.x - jacketlbound2.x)^2 + (centroidyellow2.y - jacketlbound2.y)^2));
			distjacketuboundlbound[j] = sqrt(abs((jacketubound2.x - jacketlbound2.x)^2 + (jacketubound2.y - jacketlbound2.y)^2));
			distjacketleftright[j] = sqrt(abs((jacketleft2.x - jacketright2.x)^2 + (jacketleft2.y - jacketright2.y)^2));
			distgreenyellow[j] = sqrt(abs((centroidgreen2.x - centroidyellow2.x)^2 + (centroidgreen2.y - centroidyellow2.y)^2));
		}

		// Compute the sum of all the distances
		samplecount = 0, d1 = 0, d2 = 0, d3 = 0, d4 = 0, d5 = 0, d6 = 0, d7 = 0, d8 = 0, d9 = 0;
		for (int j=0; j<NFRAMES; j++) {
			// If there is data in the frame
			if ((centroidyellow2.x > 0) && (centroidgreen2.x > 0) && (centroidjacket2.x > 0) && (jacketubound2.x > 0) && (jacketlbound2.x > 0) && (jacketleft2.x > 0) && (jacketright2.x > 0)) {
				d1 = d1 + distgreenjacketubound[j];
				d2 = d2 + distgreenjacketcentroid[j]; //
				d3 = d3 + distgreenjacketlbound[j];
				d4 = d4 + distyellowjacketubound[j];
				d5 = d5 + distyellowjacketcentroid[j]; //
				d6 = d6 + distyellowjacketlbound[j];
				d7 = d7 + distjacketuboundlbound[j];
				d8 = d8 + distjacketleftright[j];
				d9 = d9 + distgreenyellow[j]; //
				//increment sample counter
				samplecount = samplecount + 1;
			}
		}
		// Compute the mean (average) of all the distances
		d1 = d1 / samplecount;
		d2 = d2 / samplecount;
		d3 = d3 / samplecount;
		d4 = d4 / samplecount;
		...
		//Update the framectr for storing the recent NFRAMES
		if (framectr == NFRAMES-1) {
			framectr = 0; //restart counter again
		} else {
			framectr = framectr + 1; //else, increment counter
		}
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//
		...
		///////////////////////////////////
		cvShowImage("Screen", displayImage);
		...
		cvReleaseImage(&imageOrange1);
		cvReleaseImage(&imageOrange2);
