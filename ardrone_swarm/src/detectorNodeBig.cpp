...
class StateMachine
{
	...
	public: StateMachine(){
				...
				gesture_pub = nh_.advertise<std_msgs::Int8>("/ardrone/gest", 1);
                led_pub = nh_.advertise<ardrone_swarm::led>("/drone1/led", 1);
				start = -1;
				stage = STAGE_1;
            ...
            ardrone_swarm::led prepare_word(ardrone_swarm::led& msg, int id){
                msg.command = "off";
                if (id == 1){
                    msg.duration = 0.5;
                    msg.iterations = 1;
