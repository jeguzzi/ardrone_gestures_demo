...
int process_image(cv::Mat inputImage, bool takenoff, bool marker_detected, double xc, double yc, double dim) {
		...
		if (stage < STAGE_1){
			...
			if (stage == STAGE_FLOOR){
                ...
                //blobsColor.Filter(blobsColor, B_EXCLUDE, CBlobGetArea(), B_LESS, ColorBlob1.Area());

	            //Compute blob properties
                area = ColorBlob1.Area();
				//cout << "after area " << endl;
	            majoraxislength = ColorBlob1.GetEllipse().size.width;
				//cout << "after maj "<<endl;
	            minoraxislength = ColorBlob1.GetEllipse().size.height;
				//cout << "after maj "<<endl;
	            perimeter = ColorBlob1.Perimeter();

	            //Compute features
				if (minoraxislength > 0 and majoraxislength > 0){
                ...
                cout << circlescore << endl; 
				if (circlescore > 1.5) return YOU;
				else return NO_GEST;}
				else return NO_GEST;}
				else return NO_GEST;
			}			
